...
public class TeamDelete extends javax.swing.JFrame {
    ...
    private void btnFindActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnFindActionPerformed
        ...
        if (teamIdStr.isEmpty()) {
         ...
         try {
            ...
            txtTeamManager.setText(team.getManager_name());
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        ...
        try{
            Long team_id=Long.parseLong(teamIdStr);
            
            int response=JOptionPane.showConfirmDialog(this, "Apakah Anda yakin ingin menghapus tim ini?", "Konfirmasi", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            
            if(response==JOptionPane.YES_OPTION){
                teamAdm.dropTeam(team_id);
    ...
    public Team getTeamById(Long team_id) throws SQLException {
    Team team = null;
    Connection conn = DBConnection.getConnection();
    String query = "SELECT t.team_name, m.manager_name FROM TeamList t JOIN ManagerList m ON t.manager_id = m.manager_id WHERE t.team_id = ?";
    
    try (PreparedStatement ps = conn.prepareStatement(query)) {
        ps.setLong(1, team_id);
        try (ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                team = new Team(); // Anggap kamu punya kelas Team untuk menampung data tim
                team.setTeam_name(rs.getString("team_name"));
                team.setManager_name(rs.getString("manager_name"));  // Misalkan manager_name adalah field dari objek Team
            } else {
